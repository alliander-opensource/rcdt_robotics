...
class PantherFullTests:
    ...
    def test_e_stop_request(self, test_node: Node, timeout: int) -> None:
        assert (
            call_trigger_service(
                node=test_node,
    ...
    def test_switch_joy_to_panther_topic(self, test_node: Node, timeout: int) -> None:
        assert_joy_topic_switch(
            node=test_node,
    ...
    def test_move_panther_with_joy(self, test_node: Node, timeout: int) -> None:
    ...
    def test_rotate_panther_with_joy(self, test_node: Node, timeout: int) -> None:
        def compare_fn(p1: Pose, p2: Pose) -> float:
