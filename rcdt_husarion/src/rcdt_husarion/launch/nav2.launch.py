...
def launch_setup(context: LaunchContext) -> list:  # noqa: PLR0915
    ...
    return [
        ...
        map_server if use_map_localization else SKIP,
        amcl if use_map_localization else SKIP,
        controller_server if use_navigation else SKIP,
        planner_server if use_navigation else SKIP,
        behavior_server if use_navigation else SKIP,
        bt_navigator if use_navigation else SKIP,
        waypoint_follower if use_navigation else SKIP,
        collision_monitor_node
        if use_collision_monitor
        else SKIP,
        lifecycle_manager if lifecycle_nodes
        else SKIP,
        waypoint_follower_controller if use_navigation
        else SKIP,
    ]
