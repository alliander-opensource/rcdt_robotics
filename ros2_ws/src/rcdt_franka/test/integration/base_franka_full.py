...
class FrankaFullTests:
    ...
    def test_moveit_node_activated(self) -> None:
        """Test that joint states are published. This is a basic test to check that the
        launch file is working and that the robot is publishing joint states."""
    ...
    def test_gripper_action(
        self, service: str, expected_value: float, singleton_node: rclpy.node.Node
    ) -> None:
    ...
    def test_joy_gripper_node(
        self, buttons: list[int], expected_value: float, singleton_node: rclpy.node.Node
    ) -> None:
